 ...
 *
 *    File   : main.c
 *    Purpose: main for Pygmy Seven Segment Display Driver
 *                                                          
 *=========================================================*/
...
//Set GPIO direction
    ...
    PyHal_GPIO_SetDir(23,0); // MSB : U
    PyHal_GPIO_SetDir(25,0);
    PyHal_GPIO_SetDir(27,0); 
    PyHal_GPIO_SetDir(29,0);  // LSB : X
    int u, v, w, x, out;	
    ...
    while(1)
    {
        ...
        w = PyHal_GPIO_Get(27);
        x = PyHal_GPIO_Get(29);
        out = (!((!u)&(!x)))&(!(u&(!v)))&(!(v&w))&(!((!u)&(!v)&(!w)))&(!((!u)&(!w)&x));
        //out = (!(!(u))&(!(x)))&(!(u&(!(v))))&(!(v&w))&(!(!(u))&(!(v))&(!(w)))&(!(!(u))&(!(w))&x);
        if(out == 1)
    	sevenseg(1,0,0,1,1,1,1);//The number 1
    	else
    	sevenseg(0,0,0,0,0,0,1);//The number 0
    }      
